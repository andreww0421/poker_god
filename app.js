...
// [NEW] Haptic State
let hapticEnabled = localStorage.getItem('pg_haptic') !== 'false'; // Default true

window.onload = () => { 
    initTheme(); updateText(); genGrid(); genSelUI(); updateRanges(); 
    document.getElementById('brDate').valueAsDate = new Date(); 
    updateNameList(); renderBankrollUI(); 
    updateHapticUI(); // Init Haptic UI
...
function switchTab(id, el) {
    ...
    triggerHaptic(10); // [NEW] Haptic on tab switch
...
function updateText() {
    ...
    updateRanges(); 
    updateHapticUI(); // Refresh text language
}

// [NEW] Haptic Functions
function toggleHaptic() {
    hapticEnabled = !hapticEnabled;
    localStorage.setItem('pg_haptic', hapticEnabled);
    updateHapticUI();
    if(hapticEnabled) triggerHaptic(50); // Feedback when turned on
}

function updateHapticUI() {
    const label = document.getElementById('hapticLabel');
    const icon = document.getElementById('hapticIcon');
...
function setOppAction(act, btn) { state.oppAction = act; document.querySelectorAll('.btn-opp').forEach(b => b.classList.remove('active')); btn.classList.add('active'); triggerHaptic(10); } // Haptic added
...
function analyzePostflop() {
    ...
    triggerHaptic(20); // Haptic on analyze
...
function drawChart(dataProfit) { 
    ...
    if(dataProfit.length < 2) { 
        ctx.fillStyle="#94a3b8"; ctx.font="14px Arial"; ctx.textAlign="center"; ctx.fillText("No Data", w/2, h/2); 
        return; 
    } 
    let ys = dataProfit.map(p=>p.y); 
    let max = Math.max(0, ...ys); let min = Math.min(0, ...ys); 
    let range = max - min; if(range === 0) range = 100; 
    max += range * 0.1; min -= range * 0.1; 
    let mLeft=40, mBottom=30, mTop=20, mRight=10; 
    let graphW = w - mLeft - mRight; let graphH = h - mBottom - mTop; 
    ...
    ctx.beginPath(); ctx.strokeStyle = profitColor; ctx.lineWidth = 3; ctx.setLineDash([]); ctx.moveTo(mapX(0), mapY(dataProfit[0].y)); for(let i=1; i<dataProfit.length; i++) ctx.lineTo(mapX(i), mapY(dataProfit[i].y)); ctx.stroke();
